export default function FilterProfile({userName,designation,skill,about,location}){
  console.log("inside FilterProfile ",userName,designation,skill,about,location)
    return(
    <div id="Profile-block" class="  flex  flex-col gap-4 h-fit w-[85vw] p-5  rounded-md items-center text-white bg-[#ffffff79] sm:w-auto sm:h-[40vh]">
        <div id="top "class="grid grid-cols-2 grid-rows-2 w-full   justify-items-center items-center  ">
          <div id="userName" class="row-start-1 row-span-1 h-4 Rajdhani text-3xl tracking-widest
          ">{userName}</div>
          <div id="Designation" class="row-start-2 row-span-1 h-4 text-xl text-[#72FFE6] ">{designation}</div>
      ...
      <div id="mid1" class="flex  justify-between px-12  w-full   text-[#3770FF]  text-center md:px-0 md:gap-1">
        ...
        <div id="mid2" classuserName=" sm:h-[50ch]">
         {about}
      </div>
